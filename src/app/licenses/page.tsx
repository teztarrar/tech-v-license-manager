...
import { Modal, Button, Input, Select, Badge, Card, EmptyState, PageSpinner } from '@/components/ui'
...
export default function LicensesPage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(false)
  }, [search, filterStatus, filterCompany, filterProduct])
  ...
  async function handleSave() {
    ...
    try {
      ...
      else { const e = await res.json(); alert(e.error || 'Error saving license') }
  ...
  async function handleDelete() {
    ...
    setSaving(true)
    await fetch(`/api/licenses/${showDelete.id}`, { method: 'DELETE' })
  ...
  async function handleRenew() {
    if (!showRenew) return
    setSaving(true)
    await fetch('/api/licenses/renew', {
  ...
  return (
    <DashboardLayout title="License Manager">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <div className="flex gap-2 flex-wrap">
            <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)}
              className="px-3 py-2 bg-input border border-input-border rounded-lg text-sm text-input-text focus:outline-none focus:ring-2 focus:ring-brand-500/40">
      ...
      <Modal
        ...
        onClose={closeModal}
        title={showEdit ? 'Edit License' : 'Add New License'}
        size="lg"
      >
        <div className="space-y-4">
            ...
            <div className="space-y-1">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-2.5 bg-input border border-input-border rounded-lg text-sm text-input-text font-mono placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-brand-500/50"
                />
                <button
                  type="button"
                  onClick={() => setForm({ ...form, licenseKey: generateLicenseKey() })}
                  className="px-3 py-2 bg-brand-600/20 text-brand-400 rounded-lg hover:bg-brand-600/30 transition-colors text-xs font-medium whitespace-nowrap"
                >
                  Generate
                </button>
              </div>
          ...
          <Input
            ...
            placeholder="Server Room / Dept / Remote"
          />
          ...
          <div className="space-y-1">
            ...
            <textarea
              ...
              className="w-full px-3 py-2.5 bg-input border border-input-border rounded-lg text-sm text-input-text placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-brand-500/50 resize-none"
            />
          </div>
          ...
          <div className="flex justify-end gap-3 pt-2 border-t border-modal-border">
            ...
            <Button onClick={handleSave} loading={saving}>
              {showEdit ? 'Save Changes' : 'Add License'}
            </Button>
          </div>
        </div>
      </Modal>
      ...
      <Modal open={!!showRenew} onClose={() => setShowRenew(null)} title="Renew License" size="sm">
        <div className="space-y-4">
          <div className="bg-brand-500/10 border border-brand-500/20 rounded-xl p-4">
            ...
            {showRenew?.expiryDate && (
              <p className="text-xs text-gray-500 mt-1">
                Current expiry: {formatDate(showRenew.expiryDate)}
              </p>
            )}
          </div>
          <Select
          ...
          <div className="flex justify-end gap-3 pt-2 border-t border-modal-border">
            ...
            <Button variant="success" onClick={handleRenew} loading={saving} icon={<RefreshCw size={13} />}>
              Renew License
            </Button>
          </div>
        </div>
      ...
      <Modal open={!!showDelete} onClose={() => setShowDelete(null)} title="Delete License" size="sm">
        <div className="space-y-4">
          <p className="text-gray-400 text-sm">
            Are you sure you want to delete this license? This action <strong className="text-white">cannot be undone</strong>.
          </p>
          <div className="bg-red-500/10 border border-red-500/20 rounded-lg p-3 font-mono text-xs text-red-400 break-all">
            {showDelete?.licenseKey}
          </div>
          <div className="flex justify-end gap-3 pt-2 border-t border-modal-border">
            <Button variant="secondary" onClick={() => setShowDelete(null)}>Cancel</Button>
            <Button variant="danger" onClick={handleDelete} loading={saving} icon={<Trash2 size={13} />}>
              Delete License
            </Button>
          </div>
        </div>
          ...
          {importResult && (
            <>
              <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-3 text-sm text-green-400">
                ✅ {importResult.created} license{importResult.created !== 1 ? 's' : ''} imported successfully
              </div>
